...
Vagrant.configure("2") do |config|
  ...
  (1..$num_nodes).each do |i|
    config.vm.define "spark-node#{i}" do |node| # VM 名稱
      ...
      node.vm.synced_folder ".", "/share", type: "virtualbox"
      ...
      if i == 1
        ...
        node.vm.provision "shell", path: "scripts/init-worker.sh"
      end

      node.vm.provision "shell", path: "scripts/bootstrap.sh", args: [ip, "#{$masterIP}", "#{$num_nodes}"]
    end
  end
end